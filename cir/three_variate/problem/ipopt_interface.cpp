...
CIR_MLE_NLP::CIR_MLE_NLP() {
    ...
    lowPrices = new std::vector<double>;
    highPrices = new std::vector<double>;
    ...
    csvReader.readDataColumn("low", lowPrices);
    csvReader.readDataColumn("high", highPrices);
...
bool CIR_MLE_NLP::eval_f(Ipopt::Index n,
    ...
    for (Ipopt::Index i = 0; i < closePrices->size(); i++) {
        int err = model(libData,
                        closePrices->at(i),
                        openPrices->at(i),
                        tail_normalized(lowPrices->at(i), x[SIGMA]),
                        tail_normalized(highPrices->at(i), x[SIGMA]),
                        lowPrices->at(i),
                        highPrices->at(i),
                        x[SIGMA],
                        x[THETA],
...
double CIR_MLE_NLP::tail_normalized(double s, double sigma) {
    return 2.0 * sqrt(s) / sigma;
}